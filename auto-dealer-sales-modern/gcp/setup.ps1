...
$existingRepo = gcloud artifacts repositories list `
    ...
    --filter="name~/$GcpArRepo$" --format="value(name)"
if ([string]::IsNullOrWhiteSpace($existingRepo)) {
    ...
    Assert-GcloudOk "create Artifact Registry repo"
...
if ([string]::IsNullOrWhiteSpace($existingInstance)) {
    ...
    Assert-GcloudOk "create Cloud SQL instance"
...
$existingUser = gcloud sql users list --instance=$GcpSqlInstance --project=$GcpProjectId `
    --filter="name=$GcpSqlUser" --format="value(name)"
...
$existingJwt = gcloud secrets list --project=$GcpProjectId `
    --filter="name~/$GcpJwtSecretName$" --format="value(name)"
...
$existingSa = gcloud iam service-accounts list --project=$GcpProjectId `
    --filter="email=$GcpServiceAccountEmail" --format="value(email)"
if ([string]::IsNullOrWhiteSpace($existingSa)) {
    ...
    Assert-GcloudOk "create service account"
...
foreach ($role in $roles) {
    ...
    Assert-GcloudOk "bind role $role"
}
